// Questa funzione viene invocata da index.html quando gsi/client è pronto
function initApp() {
  const categorie = [
  ...
  // Inizializzo il tokenClient solo ora che google.accounts è definito
  const tokenClient = google.accounts.oauth2.initTokenClient({
    client_id: CLIENT_ID,
    scope: "openid email https://www.googleapis.com/auth/spreadsheets",
    callback: resp => {
      if (resp.error) {
        console.error("Errore login OAuth:", resp);
        alert("Login fallito: controlla la console per dettagli.");
        return;
      }
  ...
  // Fallback per forzare il consenso
  document.getElementById("login-google")
    .addEventListener("click", () => {
      tokenClient.requestAccessToken({ prompt: "consent" });
    });

  // Genera cards e select categorie
  ...
  document.getElementById("apri-budget")
    .addEventListener("click", () => {
  ...
  document.getElementById("form-budget")
  ...
  document.getElementById("spesa-form")
    .addEventListener("submit", e => {
      e.preventDefault();
      const raw     = document.getElementById("importo").value.trim();
      const valido  = raw.replace(",", ".");
      const importo = parseFloat(valido);
      if (isNaN(importo)) {
      ...
      totaleRimanente.textContent = (totNow - importo).toFixed(2);

      // Invio spesa con email utente
  ...
  function inviaSpesaASheet(cat, imp, desc) {
    ...
    fetch(
      ...
      .then(() =>
        console.log("Spesa salvata con utente:", window.userEmail)
      )
